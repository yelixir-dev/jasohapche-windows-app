...
이 저장소의 패키지는 **자소합체 Windows v0.1.0 실행 배포본**입니다.

- 이 저장소에서 제공하는 zip과 실행 파일은 사용자가 내려받아 실행하기 위한 배포물입니다.
- 앱 아이콘, 트레이 아이콘, 스크린샷 등 포함 자산은 이 배포본 안내와 실행에 필요한 범위에서 함께 제공됩니다.
- 패키지를 수정하거나 재배포하려면 배포자 표기와 무결성 정보가 달라질 수 있으므로, 기존 SHA-256 값을 그대로 안내하지 마세요.
- 별도 라이선스 파일이 추가되기 전까지, 이 저장소의 파일을 제3자 배포물에 포함하거나 재패키징하는 것은 권장하지 않습니다.

© yElixir. All rights reserved.
