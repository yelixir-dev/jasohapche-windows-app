...
1. 이 저장소를 다운로드하거나 압축 파일로 내려받습니다.
2. `v0.1.0\Jasohapche.Windows\Jasohapche.Windows.exe`를 실행합니다.
3. 필요한 감시 폴더를 추가합니다.
4. 바로 정리하려면 `지금 검사`를 누릅니다.
5. 창을 닫아도 앱은 시스템 트레이에서 계속 동작합니다.
6. 완전히 종료하려면 트레이 아이콘을 우클릭한 뒤 `종료`를 선택합니다.
